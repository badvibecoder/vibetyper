// vibetyper zig dictionary data — algorithms.
...
export const blocks = [
  String.raw`fn binary_search(sorted: []const i32, target: i32) ?usize {
    var lo: usize = 0;
  ...
  String.raw`fn insertion_sort(values: []i32) void {
    var i: usize = 1;
  ...
  String.raw`fn selection_sort(values: []i32) void {
    var i: usize = 0;
    while (i < values.len) : (i += 1) {
        var best = i;
        var j = i + 1;
  ...
  String.raw`fn merge_sort_alloc(allocator: std.mem.Allocator, values: []const i32) ![]i32 {
  ...
  String.raw`fn quick_sort(values: []i32) void {
  ...
  String.raw`fn counting_sort_alloc(allocator: std.mem.Allocator, values: []const u32, max_value: u32) ![]u32 {
  ...
  String.raw`fn bfs_can_reach(allocator: std.mem.Allocator, graph: []const []const usize, start: usize, goal: usize) !bool {
  ...
  String.raw`fn dfs_has_cycle(allocator: std.mem.Allocator, graph: []const []const usize) !bool {
  ...
  String.raw`fn connected_components(allocator: std.mem.Allocator, graph: []const []const usize) !usize {
    const visited = try allocator.alloc(bool, graph.len); defer allocator.free(visited);
    @memset(visited, false);
    const Flood = struct {
  ...
  String.raw`fn dijkstra_shortest(allocator: std.mem.Allocator, graph: []const []const u64, start: usize) ![]u64 {
  ...
  String.raw`fn knapsack_01(allocator: std.mem.Allocator, weights: []const u64, values: []const u64, capacity: usize) !u64 {
  ...
  String.raw`fn coin_change_min(allocator: std.mem.Allocator, coins: []const u32, amount: usize) !?usize {
  ...
  String.raw`fn lcs_length(allocator: std.mem.Allocator, a: []const u8, b: []const u8) !usize {
  ...
  String.raw`fn lis_length(allocator: std.mem.Allocator, values: []const i32) !usize {
  ...
  String.raw`fn kadane_max_subarray(values: []const i32) i32 {
    var best = values[0];
  ...
  String.raw`fn two_sum_sorted(sorted: []const i32, target: i32) ?struct { i: usize, j: usize } {
    var lo: usize = 0;
  ...
  String.raw`fn pair_with_sum(allocator: std.mem.Allocator, values: []const i32, target: i32) !?struct { i: usize, j: usize } {
  ...
  String.raw`fn next_permutation(values: []i32) bool {
    var i = values.len - 1;
  ...
  String.raw`fn rotate_matrix_90(allocator: std.mem.Allocator, matrix: []const []const i32) ![][]i32 {
    const n = matrix.len;
  ...
  String.raw`fn spiral_order_alloc(allocator: std.mem.Allocator, matrix: []const []const i32) !std.ArrayList(i32) {
  ...
  String.raw`fn matrix_multiply_alloc(allocator: std.mem.Allocator, a: []const []const f64, b: []const []const f64) ![][]f64 {
    const m = a.len;
    const n = b.len;
    const p = b[0].len;
  ...
  String.raw`fn binary_search_rotated(values: []const i32, target: i32) ?usize {
    var lo: usize = 0;
  ...
  String.raw`fn fibonacci_memoized(n: usize, memo: []?u64) u64 {
  ...
  String.raw`fn partition_lomuto(values: []i32, lo: usize, hi: usize) usize {
