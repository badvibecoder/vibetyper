// vibetyper zig dictionary data — input validation
export const blocks = [
  String.raw`fn is_valid_email(address: []const u8) bool {
  ...
  String.raw`fn is_strong_password(password: []const u8) bool {
  ...
  String.raw`fn is_valid_us_phone(text: []const u8) bool {
    var digits: usize = 0;
    for (text) |c| {
        if (c >= '0' and c <= '9') {
            digits += 1;
  ...
  String.raw`fn is_valid_ipv4(address: []const u8) bool {
    var parts: usize = 0;
  ...
  String.raw`fn is_valid_hex_color(text: []const u8) bool {
  ...
  String.raw`fn is_valid_username(name: []const u8) bool {
  ...
  String.raw`fn is_valid_port(text: []const u8) bool {
    const port = std.fmt.parseInt(u32, text, 10) catch return false;
    return port >= 1 and port <= 65535;
}`,

  String.raw`fn is_valid_date_ymd(year: u32, month: u32, day: u32) bool {
  ...
  String.raw`fn is_leap_year(year: u32) bool {
    if (year % 400 == 0) return true;
    if (year % 100 == 0) return false;
    return year % 4 == 0;
}`,

  String.raw`fn is_valid_luhn(digits: []const u8) bool {
  ...
  String.raw`fn is_balanced_parentheses(text: []const u8) bool {
    var depth: usize = 0;
    for (text) |c| {
        if (c == '(') {
            depth += 1;
  ...
  String.raw`fn contains_only_digits(text: []const u8) bool {
  ...
  String.raw`fn is_valid_slug(text: []const u8) bool {
  ...
  String.raw`fn is_valid_semver(version: []const u8) bool {
    var dots: usize = 0;
    var prev_dot = false;
    for (version, 0..) |c, i| {
        if (c == '.') {
  ...
  String.raw`fn is_valid_percent(text: []const u8) bool {
    const value = std.fmt.parseInt(u8, text, 10) catch return false;
    return value <= 100;
}`,

  String.raw`fn is_ascii_printable(text: []const u8) bool {
    if (text.len == 0) return false;
    for (text) |c| {
  ...
  String.raw`fn is_valid_domain_label(label: []const u8) bool {
  ...
  String.raw`fn is_valid_http_url(url: []const u8) bool {
  ...
  String.raw`// Latitude and longitude must stay within their physical bounds.
fn is_valid_coordinates(lat: f64, lon: f64) bool {
    return lat >= -90.0 and lat <= 90.0 and lon >= -180.0 and lon <= 180.0;
}`,

  String.raw`fn is_valid_hex_str(text: []const u8) bool {
  ...
  String.raw`fn is_valid_identifier(name: []const u8) bool {
  ...
  String.raw`fn is_valid_hhmm(text: []const u8) bool {
