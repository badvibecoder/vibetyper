// vibetyper rust dictionary data — algorithms.
// Each entry is ONE complete, balanced top-level Rust unit.

export const blocks = [
  String.raw`fn binary_search(sorted: &[i32], target: i32) -> Option<usize> {
    let mut low = 0;
  ...
  String.raw`// Repeatedly pick the smallest remaining value and swap it into place.
fn selection_sort(items: &mut [i32]) {
  ...
  String.raw`fn insertion_sort(items: &mut [i32]) {
  ...
  String.raw`fn merge_sort(values: Vec<i32>) -> Vec<i32> {
    if values.len() <= 1 {
        return values;
    }
  ...
  String.raw`fn quicksort(items: &mut [i32]) {
  ...
  String.raw`// Counting sort for small, bounded u32 keys.
fn counting_sort(values: &[u32], max_value: u32) -> Vec<u32> {
  ...
  String.raw`fn bfs_can_reach(graph: &[Vec<usize>], start: usize, goal: usize) -> bool {
  ...
  String.raw`fn dfs_has_cycle(graph: &[Vec<usize>]) -> bool {
  ...
  String.raw`fn connected_components(graph: &[Vec<usize>]) -> usize {
    let mut seen = vec![false; graph.len()];
  ...
  String.raw`fn dijkstra_shortest(graph: &[Vec<(usize, u64)>], start: usize) -> Vec<u64> {
  ...
  String.raw`// 0/1 knapsack with a one-dimensional DP table over capacity.
fn knapsack_01(weights: &[u64], values: &[u64], capacity: u64) -> u64 {
  ...
  String.raw`fn coin_change_min(coins: &[u64], amount: u64) -> Option<usize> {
  ...
  String.raw`fn lcs_length(a: &str, b: &str) -> usize {
  ...
  String.raw`fn lis_length(values: &[i32]) -> usize {
  ...
  String.raw`// Kadane algorithm: largest sum of any contiguous subarray.
fn kadane_max_subarray(values: &[i32]) -> i32 {
  ...
  String.raw`fn two_sum_sorted(sorted: &[i32], target: i32) -> Option<(usize, usize)> {
    let mut left = 0;
  ...
  String.raw`// One pass with a hash map recording where each value was seen.
fn pair_with_sum(values: &[i32], target: i32) -> Option<(usize, usize)> {
  ...
  String.raw`fn next_permutation(values: &mut [i32]) -> bool {
  ...
  String.raw`fn rotate_matrix_90(matrix: &mut [Vec<i32>]) {
    let size = matrix.len();
  ...
  String.raw`fn spiral_order(matrix: &[Vec<i32>]) -> Vec<i32> {
  ...
  String.raw`// Naive row-by-column product; loop order is cache friendly.
fn matrix_multiply(left: &[Vec<f64>], right: &[Vec<f64>]) -> Vec<Vec<f64>> {
  ...
  String.raw`fn binary_search_rotated(values: &[i32], target: i32) -> Option<usize> {
    let mut low = 0;
  ...
  String.raw`fn fibonacci_memoized(n: usize) -> u64 {
  ...
  String.raw`// Lomuto partition; the returned index is the final position of the pivot.
