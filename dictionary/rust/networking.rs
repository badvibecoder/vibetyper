// vibetyper Rust dictionary — networking & URL helpers
...
fn url_path_segments(path: &str) -> Vec<&str> {
    path.split('/')
...
fn build_get_request_line(path: &str, host: &str) -> String {
...
fn parse_headers_to_map(header_lines: &[String]) -> std::collections::HashMap<String, String> {
...
fn ipv4_from_string(text: &str) -> Option<[u8; 4]> {
...
fn cidr_contains(ip: [u8; 4], cidr: &str) -> bool {
...
// Last two labels of a hostname, e.g. "cdn.edge.gadgetnet.io" -> "gadgetnet.io".
...
fn mime_for_extension(extension: &str) -> &str {
