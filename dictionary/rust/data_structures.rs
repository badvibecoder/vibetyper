// vibetyper Rust dictionary — data structures

// A classic LIFO stack backed by a growable vector.
...
// FIFO queue built on VecDeque so both ends stay O(1).
#[derive(Debug, Clone, PartialEq)]
struct Queue<T> {
...
// Min-heap adapter: Reverse flips BinaryHeap so the smallest value is on top.
#[derive(Debug, Clone)]
struct MinHeap {
...
// Fixed-capacity circular buffer: pushing past capacity evicts the oldest item.
#[derive(Debug, Clone)]
struct RingBuffer<T> {
    capacity: usize,
...
// Tally of named events; defaults to zero for unseen keys.
...
// Half-open time range over u64 nanoseconds.
...
// Packed bitset stored in 64-bit words.
#[derive(Debug, Clone, PartialEq)]
struct BitSet {
    words: Vec<u64>,
}
...
// Token bucket rate limiter with a continuous refill schedule.
#[derive(Debug, Clone)]
struct TokenBucket {
    capacity: u64,
    tokens: f64,
...
// Welford online update: feed one sample, get back (mean, m2, count).
...
fn classify_edge(prev: f64, current: f64, tolerance: f64) -> Option<bool> {
...
// Rolling sum over a sliding window; short inputs yield short output.
