// vibetyper Rust dictionary — hashing, encoding & time
...
fn simple_checksum(bytes: &[u8]) -> u8 {
    let mut sum: u32 = 0;
    for byte in bytes {
...
// Scramble each byte with the key; applying it twice restores the input.
fn xor_encrypt_bytes(bytes: &[u8], key: u8) -> Vec<u8> {
    bytes.iter().map(|byte| byte ^ key).collect()
}

// Compact base-36 encoding for short opaque ids.
...
fn xor_checksum_bytes(bytes: &[u8]) -> u8 {
...
// Compare without early exit so timing reveals nothing.
...
fn session_token_from_seed(mut seed: u64) -> String {
...
fn unix_to_date_parts(days: i64) -> (i32, u32, u32) {
    let z = days + 719_468;
...
fn ms_since_epoch(time: std::time::SystemTime) -> u128 {
...
fn seconds_to_hms(total: u64) -> (u32, u32, u32) {
...
fn parse_hhmm_to_minutes(text: &str) -> Option<u32> {
...
fn elapsed_seconds_between(start: u64, end: u64) -> u64 {
    if end >= start {
        end - start
    } else {
        0
...
fn time_of_day_label(hour: u32) -> String {
    match hour {
