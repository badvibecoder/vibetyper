// vibetyper Rust dictionary — data processing & parsing
...
fn count_lines_words_chars(text: &str) -> (usize, usize, usize) {
...
fn parse_integer_list(input: &str) -> Result<Vec<i64>, String> {
...
fn transpose_matrix(matrix: &[Vec<f64>]) -> Option<Vec<Vec<f64>>> {
    let rows = matrix.len();
...
fn binned_counts(values: &[f64], bin_count: usize) -> Vec<usize> {
...
fn parse_size_string(input: &str) -> Option<u64> {
...
fn split_commands_quoted(input: &str) -> Vec<String> {
...
fn find_duplicate_values(values: Vec<String>) -> Vec<String> {
    use std::collections::HashMap;
...
fn outliers(values: &[f64]) -> Vec<f64> {
    if values.is_empty() {
        return Vec::new();
    }
...
fn minmax_normalize(values: Vec<f64>) -> Vec<f64> {
...
// Replace runs of digits with a single hash mark.
...
fn sample_every_nth<T>(values: Vec<T>, step: usize) -> Vec<T> {
    if step == 0 {
        return Vec::new();
    }
    values
...
fn cumulative_average(values: &[f64]) -> Vec<f64> {
...
fn parse_http_header_line(line: &str) -> Option<(String, String)> {
