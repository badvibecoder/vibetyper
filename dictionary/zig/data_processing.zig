...
// vibetyper Zig dictionary — data processing & parsing
...
fn parse_query_string_alloc(allocator: std.mem.Allocator, query: []const u8) !std.ArrayList(QueryPair) {
...
fn parse_csv_row_alloc(allocator: std.mem.Allocator, line: []const u8) !std.ArrayList([]const u8) {
...
fn parse_key_value_line(line: []const u8) ?struct { key: []const u8, value: []const u8 } {
...
fn extract_numbers_alloc(allocator: std.mem.Allocator, text: []const u8) !std.ArrayList(f64) {
...
fn tokenize_words_alloc(allocator: std.mem.Allocator, text: []const u8) !std.ArrayList([]const u8) {
...
fn normalize_text_alloc(allocator: std.mem.Allocator, text: []const u8) ![]u8 {
...
fn count_lines_words_chars(text: []const u8) struct { lines: usize, words: usize, chars: usize } {
    var lines: usize = 1;
    var words: usize = 0;
    var in_word = false;
    for (text) |c| {
...
fn parse_integer_list_alloc(allocator: std.mem.Allocator, text: []const u8) !std.ArrayList(i64) {
...
fn transpose_matrix_alloc(allocator: std.mem.Allocator, rows: []const []const f64) ![][]f64 {
...
fn sum_by_key(allocator: std.mem.Allocator, pairs: []const struct { key: []const u8, value: i64 }) !std.StringHashMap(i64) {
...
fn binned_counts_alloc(allocator: std.mem.Allocator, values: []const f64, bin_count: usize) !std.ArrayList(usize) {
...
fn interpolate_missing_alloc(allocator: std.mem.Allocator, series: []const ?f64) ![]f64 {
...
fn parse_duration_seconds(text: []const u8) ?u64 {
    var total: u64 = 0;
    var start: usize = 0;
    var i: usize = 0;
    while (i < text.len) {
...
fn parse_size_bytes(text: []const u8) ?u64 {
    var i: usize = 0;
...
fn split_commands_quoted_alloc(allocator: std.mem.Allocator, text: []const u8) !std.ArrayList([]const u8) {
...
fn find_duplicates_alloc(allocator: std.mem.Allocator, items: []const []const u8) !std.ArrayList([]const u8) {
...
fn aggregate_counts(allocator: std.mem.Allocator, labels: []const []const u8) !std.StringHashMap(usize) {
    var counts = std.StringHashMap(usize).init(allocator);
    for (labels) |label| {
...
fn outliers_alloc(allocator: std.mem.Allocator, values: []const f64) !std.ArrayList(f64) {
...
fn minmax_normalize_alloc(allocator: std.mem.Allocator, values: []const f64) ![]f64 {
...
fn parse_coord_pair(text: []const u8) ?struct { lat: f64, lon: f64 } {
...
fn redact_numbers_alloc(allocator: std.mem.Allocator, text: []const u8) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    var in_digits = false;
    for (text) |c| {
        if (c >= '0' and c <= '9') {
...
fn sample_every_nth_alloc(allocator: std.mem.Allocator, data: []const u8, step: usize) ![]const u8 {
...
fn cumulative_average_alloc(allocator: std.mem.Allocator, values: []const f64) ![]f64 {
    const out = try allocator.alloc(f64, values.len);
    var running: f64 = 0.0;
    for (values, 0..) |v, i| {
        running += v;
