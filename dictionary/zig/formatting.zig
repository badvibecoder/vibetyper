...
// vibetyper Zig dictionary — formatting & rendering

fn byte_size_human_alloc(allocator: std.mem.Allocator, bytes: u64) ![]u8 {
...
fn duration_hms_alloc(allocator: std.mem.Allocator, seconds: u64) ![]u8 {
...
fn number_with_commas_alloc(allocator: std.mem.Allocator, value: u64) ![]u8 {
...
fn pad_start(text: []const u8, buf: []u8, width: usize, pad: u8) usize {
...
fn pad_end(text: []const u8, buf: []u8, width: usize, pad: u8) usize {
    const target = @min(width, buf.len);
    const keep = @min(text.len, target);
...
fn truncate_middle_alloc(allocator: std.mem.Allocator, text: []const u8, max_len: usize) ![]u8 {
...
fn format_percent_alloc(allocator: std.mem.Allocator, ratio: f64) ![]u8 {
    const clamped = std.math.clamp(ratio, 0.0, 1.0);
    return std.fmt.allocPrint(allocator, "{d:.1}%", .{clamped * 100.0});
}

fn format_table_row_alloc(allocator: std.mem.Allocator, cells: []const []const u8, widths: []const usize) ![]u8 {
...
fn human_list_alloc(allocator: std.mem.Allocator, parts: []const []const u8) ![]u8 {
...
fn format_elapsed_ms(allocator: std.mem.Allocator, ms: u64) ![]u8 {
...
fn format_money_cents_alloc(allocator: std.mem.Allocator, cents: u64) ![]u8 {
...
fn format_rate_alloc(allocator: std.mem.Allocator, count: u64, seconds: f64) ![]u8 {
...
fn hex_pairs_alloc(allocator: std.mem.Allocator, bytes: []const u8) ![]u8 {
...
fn format_count_alloc(allocator: std.mem.Allocator, count: u64, noun: []const u8) ![]u8 {
...
fn f64_fixed_alloc(allocator: std.mem.Allocator, value: f64, decimals: u32) ![]u8 {
...
fn format_version_alloc(allocator: std.mem.Allocator, major: u32, minor: u32, patch: u32) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    try out.writer().print("{d}.{d}.{d}", .{ major, minor, patch });
    return out.toOwnedSlice();
}

fn iso_date_alloc(allocator: std.mem.Allocator, year: u32, month: u32, day: u32) ![]u8 {
    var buf: [16]u8 = undefined;
    const s = std.fmt.bufPrint(&buf, "{d:0>4}-{d:0>2}-{d:0>2}", .{ year, month, day }) catch unreachable;
    return allocator.dupe(u8, s);
}

fn time_12h_alloc(allocator: std.mem.Allocator, hour: u32, minute: u32) ![]u8 {
...
fn sorted_map_string_alloc(allocator: std.mem.Allocator, map: std.StringHashMap(i64)) ![]u8 {
...
fn line_numbered_alloc(allocator: std.mem.Allocator, lines: []const []const u8) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    for (lines, 0..) |line, i| {
