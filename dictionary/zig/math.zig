...
// vibetyper Zig dictionary — math & numeric utilities
...
fn sieve_alloc(allocator: std.mem.Allocator, limit: usize) !std.ArrayList(bool) {
...
fn factorial_checked(n: u64) ?u64 {
    var result: u64 = 1;
    for (2..=n) |i| {
...
fn mean_f64(values: []const f64) f64 {
...
fn median_alloc(allocator: std.mem.Allocator, values: []const f64) !f64 {
...
fn standard_deviation(values: []const f64) f64 {
    if (values.len == 0) return 0.0;
...
fn percentile_alloc(allocator: std.mem.Allocator, values: []const f64, p: f64) !f64 {
    if (values.len == 0) return error.EmptyInput;
    const sorted = try allocator.dupe(f64, values);
    defer allocator.free(sorted);
    std.sort.block(f64, sorted, {}, std.sort.asc(f64));
...
fn to_base_string_alloc(allocator: std.mem.Allocator, value: u64, base: u8) ![]u8 {
...
fn moving_average_alloc(allocator: std.mem.Allocator, values: []const f64, window: usize) ![]f64 {
