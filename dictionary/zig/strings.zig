...
// vibetyper Zig dictionary — string utilities

fn to_lower_ascii(buf: []u8) void {
    for (buf) |*c| {
        if (c.* >= 'A' and c.* <= 'Z') {
            c.* += 32;
        }
...
fn count_occurrences(haystack: []const u8, needle: u8) usize {
    var count: usize = 0;
    for (haystack) |c| {
        if (c == needle) count += 1;
    }
...
fn first_word(text: []const u8) []const u8 {
    var end: usize = 0;
...
fn ends_with_str(haystack: []const u8, suffix: []const u8) bool {
    if (suffix.len > haystack.len) return false;
    return std.mem.eql(u8, haystack[haystack.len - suffix.len ..], suffix);
}

fn index_of_byte(haystack: []const u8, needle: u8) ?usize {
    for (haystack, 0..) |c, i| {
...
fn split_alloc(allocator: std.mem.Allocator, text: []const u8, sep: u8) !std.ArrayList([]const u8) {
...
fn join_alloc(allocator: std.mem.Allocator, parts: []const []const u8, sep: []const u8) ![]u8 {
    var total: usize = 0;
...
fn is_palindrome(text: []const u8) bool {
    var i: usize = 0;
...
fn caesar_alloc(allocator: std.mem.Allocator, text: []const u8, shift: u8) ![]u8 {
...
fn longest_common_prefix(left: []const u8, right: []const u8) usize {
...
fn repeat_alloc(allocator: std.mem.Allocator, ch: u8, count: usize) ![]u8 {
    const out = try allocator.alloc(u8, count);
    @memset(out, ch);
    return out;
}

fn truncate_alloc(allocator: std.mem.Allocator, text: []const u8, max_len: usize) ![]u8 {
...
fn count_letters(text: []const u8) usize {
    var count: usize = 0;
    for (text) |c| {
...
fn split_lines(allocator: std.mem.Allocator, text: []const u8) !std.ArrayList([]const u8) {
...
fn mask_card_alloc(allocator: std.mem.Allocator, card: []const u8) ![]u8 {
...
fn to_title_case_alloc(allocator: std.mem.Allocator, text: []const u8) ![]u8 {
...
fn snake_to_camel_alloc(allocator: std.mem.Allocator, ident: []const u8) ![]u8 {
...
fn rot13(text: []const u8, buf: []u8) usize {
...
fn has_unique_chars(text: []const u8) bool {
...
fn wrap_alloc(allocator: std.mem.Allocator, text: []const u8, width: usize) !std.ArrayList([]const []const u8) {
...
fn extract_digits_alloc(allocator: std.mem.Allocator, text: []const u8) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    for (text) |c| {
...
fn is_anagram(left: []const u8, right: []const u8) bool {
