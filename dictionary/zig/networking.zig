...
// vibetyper Zig dictionary — networking & URL helpers

fn parse_host_port(text: []const u8) ?struct { host: []const u8, port: u16 } {
...
fn path_segments_alloc(allocator: std.mem.Allocator, path: []const u8) !std.ArrayList([]const u8) {
...
fn find_query_param(query: []const u8, key: []const u8) ?[]const u8 {
...
fn percent_encode_alloc(allocator: std.mem.Allocator, text: []const u8) ![]u8 {
...
fn parse_status_line(line: []const u8) ?struct { code: u16, reason: []const u8 } {
...
// Produces e.g. "GET /index.html HTTP/1.1".
...
fn is_valid_http_method(method: []const u8) bool {
...
// Renders the four octets in dotted-decimal form.
...
fn ipv4_from_string(text: []const u8) ?[4]u8 {
...
fn cidr_contains(ip: [4]u8, cidr: []const u8) bool {
...
fn build_url_alloc(allocator: std.mem.Allocator, scheme: []const u8, host: []const u8, port: u16, path: []const u8) ![]u8 {
...
fn extract_registered_domain(hostname: []const u8) ?struct { domain: []const u8, tld: []const u8 } {
...
fn is_private_ipv4(ip: [4]u8) bool {
...
fn mime_for_extension(ext: []const u8) []const u8 {
...
fn parse_cookie_pair(pair: []const u8) ?struct { key: []const u8, value: []const u8 } {
...
fn build_query_string_alloc(allocator: std.mem.Allocator, pairs: []const struct { key: []const u8, value: []const u8 }) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    for (pairs, 0..) |p, i| {
...
fn sanitize_hostname_alloc(allocator: std.mem.Allocator, hostname: []const u8) ![]u8 {
    var out = std.ArrayList(u8).init(allocator);
    for (hostname) |c| {
