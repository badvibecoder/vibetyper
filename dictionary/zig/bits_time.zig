...
// vibetyper Zig dictionary — hashing, encoding & time

fn djb2_hash(text: []const u8) u32 {
    var hash: u32 = 5381;
    for (text) |c| {
        hash = hash *% 33 +% c;
    }
...
fn simple_checksum(text: []const u8) u8 {
    var sum: u8 = 0;
    for (text) |c| {
        sum +%= c;
    }
...
fn xor_encrypt_inplace(buf: []u8, key: u8) void {
    for (buf) |*b| {
        b.* ^= key;
    }
}

fn base36_encode_alloc(allocator: std.mem.Allocator, value: u64) ![]u8 {
...
fn xor_checksum_bytes(text: []const u8) u8 {
    var acc: u8 = 0;
    for (text) |c| {
        acc ^= c;
    }
...
fn constant_time_eq(left: []const u8, right: []const u8) bool {
...
fn session_token_alloc(allocator: std.mem.Allocator, seed: u64) ![]u8 {
...
fn unix_to_date_parts(days: i64) struct { year: i32, month: u32, day: u32 } {
    const z = days + 719468;
...
fn day_of_week_sakamoto(year: u32, month: u32, day: u32) u32 {
...
fn month_name(month: u32) []const u8 {
...
fn seconds_to_hms(total: u64) struct { hours: u64, minutes: u64, seconds: u64 } {
    return .{
        .hours = total / 3600,
...
fn parse_hhmm_to_minutes(text: []const u8) ?u32 {
